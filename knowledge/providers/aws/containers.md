...
- **App Mesh vs ALB-based routing** -- **App Mesh is deprecated (discontinuation September 30, 2026).** AWS recommends migrating to **ECS Service Connect** (for ECS workloads) or **Amazon VPC Lattice** (for cross-service networking across ECS, EKS, and EC2). App Mesh (Envoy-based service mesh) provided fine-grained traffic control, retries, circuit breaking, and mutual TLS between services. ALB-based routing remains an option and is sufficient for most request/response patterns. For new deployments, do not adopt App Mesh.
