...
- [ ] **[Critical]** Configure SQS visibility timeout to exceed the maximum processing time of consumers; default is 30 seconds, maximum is 12 hours; use `ChangeMessageVisibility` for dynamic extension
- [ ] **[Recommended]** Enable SQS long polling (WaitTimeSeconds 1-20 seconds) to reduce empty responses and API costs; short polling returns immediately and may miss messages distributed across servers
- [ ] **[Critical]** Set up SQS dead letter queues (DLQ) with a redrive policy (maxReceiveCount typically 3-5); configure DLQ redrive to move failed messages back to source queue after investigation
- [ ] **[Recommended]** Configure SNS message filtering policies on subscriptions to route messages by attributes, reducing consumer processing and Lambda invocations by up to 90% compared to filtering in application code
- [ ] **[Recommended]** Design EventBridge event buses with schemas: default bus for AWS service events, custom bus for application events; use schema registry and discovery for event contract management
- [ ] **[Recommended]** Configure EventBridge archive and replay for debugging and reprocessing; archive specific events with filtering rules; replay events to the same or different bus within the retention period
- [ ] **[Recommended]** Evaluate EventBridge Pipes for point-to-point integrations between event sources (SQS, Kinesis, DynamoDB Streams, MSK) and targets with optional filtering, enrichment, and transformation -- reducing custom Lambda glue code
- [ ] **[Recommended]** Choose Kinesis Data Streams shard count based on throughput: each shard supports 1 MB/s write (1,000 records/s) and 2 MB/s read; use on-demand capacity mode for unpredictable workloads
- [ ] **[Recommended]** Configure Amazon Data Firehose (formerly Kinesis Data Firehose) for zero-administration delivery to S3, Redshift, OpenSearch, or HTTP endpoints with automatic batching, compression (GZIP/Snappy), and format conversion (Parquet/ORC)
- [ ] **[Recommended]** Evaluate MSK (Managed Streaming for Apache Kafka) vs Kinesis: MSK for Kafka ecosystem compatibility and existing Kafka expertise; Kinesis for simpler operations and tighter AWS integration
- [ ] **[Recommended]** Set appropriate message retention: SQS (1 minute to 14 days, default 4 days), Kinesis (24 hours to 365 days), EventBridge archive (indefinite), MSK (configurable, broker storage-based)
- [ ] **[Critical]** Implement message deduplication: SQS FIFO uses content-based or explicit deduplication IDs (5-minute window); EventBridge and SNS provide at-least-once delivery requiring consumer-side idempotency
- [ ] **[Recommended]** Design fan-out patterns: SNS -> multiple SQS queues for parallel processing, EventBridge rules -> multiple targets (up to 5 per rule, use multiple rules for more), Kinesis enhanced fan-out for dedicated 2 MB/s per consumer
...
- **SQS Standard vs FIFO** -- Standard queues provide nearly unlimited throughput with at-least-once delivery and best-effort ordering. FIFO queues guarantee exactly-once processing and strict ordering within message groups but are limited to 3,000 messages/second with batching (70,000 with FIFO high-throughput mode). Use FIFO for financial transactions, command ordering, or anywhere duplicate processing causes harm. Use Standard for high-throughput workloads tolerant of occasional duplicates.
...
- **EventBridge Pipes vs custom Lambda glue** -- EventBridge Pipes provides managed point-to-point integrations between sources (SQS, Kinesis, DynamoDB Streams, MSK) and targets with built-in filtering, enrichment (via Lambda, Step Functions, API Gateway, API destination), and input transformation. This eliminates the need for custom Lambda functions that simply read from one service and write to another. Use Pipes for straightforward source-to-target integrations; use custom Lambda when you need complex business logic or multiple targets from a single source.
