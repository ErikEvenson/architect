...
- [ ] **[Critical]** Is the ASG spread across at least 2 AZs (3 recommended) with capacity rebalancing enabled?
- [ ] **[Recommended]** Are mixed instance policies configured with multiple instance types and purchase options (On-Demand base + Spot) for cost savings?
...
- [ ] **[Recommended]** Are target tracking scaling policies configured for CPU, request count, or custom CloudWatch metrics with appropriate cooldowns?
- [ ] **[Recommended]** Is lifecycle hook configured for graceful drain on scale-in, allowing in-flight requests to complete?
- [ ] **[Recommended]** Are instance health checks using ELB health checks (not just EC2 status checks) for accurate replacement of unhealthy instances?
- [ ] **[Recommended]** Is SSM Session Manager used for shell access instead of SSH bastion hosts, eliminating port 22 exposure?
