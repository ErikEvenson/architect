...
- [ ] **[Critical]** Is the management domain sized correctly with a minimum of 4 ESXi hosts (for vSAN), running vCenter, SDDC Manager, NSX Manager cluster (3 nodes), and Aria Suite, with sufficient headroom so management workloads do not contend with workload domain resources?
...
- [ ] **[Critical]** Is the VCF compatibility matrix (kb.vmware.com) checked before any hardware purchase or upgrade to confirm that server model, ESXi version, vSAN version, NSX version, and NIC firmware are all validated for the target VCF release?
...
- [ ] **[Recommended]** Are vCenter, NSX Manager, and Aria Suite backups coordinated with SDDC Manager backups so that a full restore scenario can recover the entire management domain to a consistent state?
...
- **Management domain sizing: 4 hosts vs 5+ hosts** — 4 hosts is the minimum for vSAN but leaves little headroom for management workload growth (Aria Suite, Tanzu Supervisor); 5+ hosts provides N+1 availability and capacity for adding Aria Operations, Aria Automation, and Tanzu services
...
- **Upgrade cadence: every release vs LTS** — upgrading every VCF release (quarterly) for latest features and security patches vs waiting for Long-Term Support releases (annual) for stability; VCF upgrade paths sometimes require stepping through intermediate versions, making deferred upgrades more time-consuming
...
Phase 3: Post-Bring-Up
  ...
  └── Deploy Aria Suite (Operations, Logs, Automation) if licensed
```
