...
router = APIRouter(prefix="/versions/{version_id}/adrs", tags=["adrs"])
...
async def _next_adr_number(version_id: uuid.UUID, session: AsyncSession) -> int:
    result = await session.execute(
        select(func.coalesce(func.max(ADR.adr_number), 0)).where(ADR.version_id == version_id)
    )
    return result.scalar() + 1


@router.get("", response_model=list[ADRResponse])
async def list_adrs(version_id: uuid.UUID, session: AsyncSession = Depends(get_session)):
...
async def create_adr(
    version_id: uuid.UUID, data: ADRCreate, session: AsyncSession = Depends(get_session)
):
    await _get_version(version_id, session)
    adr_number = await _next_adr_number(version_id, session)
...
async def get_adr(
    version_id: uuid.UUID, adr_id: uuid.UUID, session: AsyncSession = Depends(get_session)
):
    await _get_version(version_id, session)
    adr = await session.get(ADR, adr_id)
    if not adr or adr.version_id != version_id:
...
async def supersede_adr(
    ...
    if not old_adr or old_adr.version_id != version_id:
        raise HTTPException(status_code=404, detail="ADR not found")
